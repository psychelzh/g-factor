...
ggsave(
  fs::path(path_ms_fig, "p_single.png"), p_single, 
  width = 8, height = 6
)
```
...
p <- p_rel / p_cor_ramp +
  plot_annotation(tag_levels = "A")
ggsave(fs::path(path_ms_fig, "task_influence_behav_only.png"), p, width = 10, height = 8)
```
...
for ax, title_col in zip(axes[0], title_cols):
  ax.set_title(title_col, size='xx-large')
