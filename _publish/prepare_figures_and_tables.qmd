...
p_hist <- indices |>
  ggplot(aes(score)) +
  geom_histogram() +
  facet_wrap(~ task_index, scales = "free_x") +
  ggpubr::theme_pubr()
ggsave(fs::path(path_ms_fig, "hist_all_tasks.png"), p_hist, width = 10, height = 8)
```
...
cor_rapm_gca <- tar_read(scores_spearman, store = store_preproc_behav) |>
  inner_join(
...
p_cor_ramp <- cor_rapm_sampled |>
  ...
  ggdist::stat_dots(side = "left") +
  geomtextpath::geom_texthline(
    yintercept = cor_rapm_gca$r,
    hjust = 0,
    linetype = "dashed",
    label = "All"
  ) +
  scale_x_continuous(breaks = scales::breaks_width(3)) +
  rlang::exec(scale_color_brewer, !!!scale_meas_trait, palette = "Accent") +
  ggpubr::theme_pubr() +
...
p_cor_ramp_fisher <- cor_rapm_sampled |>
  ggplot(aes(num_vars, r_fisher)) +
  ggdist::stat_slabinterval() +
  ggdist::stat_dots(side = "left") +
  geomtextpath::geom_texthline(
    yintercept = cor_rapm_gca$r_fisher,
    hjust = 0,
    linetype = "dashed",
    color = "black",
    label = "All"
  ...
  scale_x_continuous(breaks = scales::breaks_width(3)) +
  ggpubr::theme_pubr() +
  labs(x = "Number of Tasks", y = "Correlation with RAPM (Fisher Transformed)")
ggsave(fs::path(path_ms_fig, "task_influence_fisher.png"), p_cor_ramp_fisher, width = 6, height = 4)
```
...
p_pred_trend <- preds_sampled |>
  ...
  ggdist::stat_dots(side = "left") +
  geom_hline(
    aes(color = trait, yintercept = estimate),
    linetype = "dashed",
    preds_main
  ) +
  scale_x_continuous(breaks = scales::breaks_width(3)) +
  rlang::exec(
    scale_colour_brewer, !!!scale_meas_trait,
    palette = "Accent"
  ) +
  ggpubr::theme_pubr() +
...
dice_clean <- targets::tar_read(
  ...
  mutate(dice_avg = tanh((atanh(pos_dice) + atanh(neg_dice)) / 2)) |>
  filter(
    parcel == "nn268",
