...
# helper functions
arrange_plots <- function(plot_fun, adj_mat_pool, ..., cex_title = 1) {
  add_custom_title <- function(text, where = c("row", "column")) {
    ...
    mtext(text, side = side, line = 0, at = at, outer = TRUE, cex = cex_title)
  }
...
p_single <- p_loading / p_expvars + plot_annotation(tag_levels = "A")
ggsave(
  fs::path(path_ms_fig, "fig-supp-3.png"), p_single, 
  width = 8, height = 6
)
```
...
p_parcel <- p_hypers_parcel / p_cond_parcel +
  ...
  plot_annotation(tag_levels = "A") &
  theme(legend.position = "bottom")
ggsave(fs::path(path_ms_fig, "fig-supp-4.png"), p_parcel, width = 8, height = 6)
...
p_dice_supp <- dice_clean_supp |>
  ...
  labs(x = "Number of Tasks", y = "Dice Similarity")
ggsave(fs::path(path_ms_fig, "fig-supp-5.png"), p_dice_supp, width = 8, height = 6)
```
