...
p_comp_rel <- tar_read(comp_rel, store = store_g_invariance) |>
  ...
  geomtextpath::geom_texthline(
    yintercept = tar_read(comp_rel_bifac, store = store_preproc_behav)[[1]],
    linetype = "dotted",
    hjust = 0,
    color = "black",
    label = "All"
...
cor_rapm_gca <- tar_read(scores_bifac, store = store_preproc_behav) |>
  inner_join(
    tar_read(scores_rapm, store = store_preproc_behav),
    by = "sub_id"
  ) |>
  summarise(r = cor(g, RAPM, use = "pairwise"), r_fisher = atanh(r))
...
datasummary(
  Factor(num_vars) ~ r * (Mean + SD + Min + Max),
  cor_rapm_sampled,
  fmt = 3,
  output = fs::path(path_ms_tbl, "cor_rapm_sampled.xlsx")
)

# pairwise correlation and correlation with RAPM
...
p_rel <- tar_read(scores_g, store = store_g_invariance) |>
