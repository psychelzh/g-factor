...
tasks_high_cor_rapm <- data_names |>
  inner_join(
    cor_rapm_sampled |>
...
tasks_high_pred |>
  ...
  geom_text_wordcloud_area(seed = 1) +
  scale_size_area(max_size = 8) +
  facet_grid(vars(edge_type), vars(num_vars),
             labeller = labeller(
               edge_type = c(all = "Combined",
                             neg = "Anti-Corr Networks",
                             pos = "Pos-Corr Networks")
...
## Estimation Stability {.smaller}

```{r}
#| label: fig-g-stability
#| fig-width: 8
#| fig-height: 6
#| fig-cap: >
