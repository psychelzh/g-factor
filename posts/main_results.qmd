...
library(brainconn)
library(lavaan)
Sys.setenv("TAR_PROJECT" = "project_g_invariance")
edge_types <- c("pos", "neg", "all")
```
...
subjs_fmri <- targets::tar_read(fc_data_matched) |>
  select(sub_id) |>
...
targets::tar_load(c(var_exp, var_exp_full))
var_exp |>
...
targets::tar_load(c(scores_g, scores_g_full))
...
targets::tar_load(c(cpm_pred, cpm_pred_main))
idx_name <- c(g_full = "All Tasks", rapm = "RAPM")
preds_main <- cpm_pred_main |> 
  summarise(across(all_of(edge_types), mean), .by = idx) |> 
  pivot_longer(
    all_of(edge_types),
    names_to = "edge_type",
    values_to = "cor"
  ) |> 
  mutate(idx_name = idx_name[idx])
preds_sampled <- cpm_pred |> 
  summarise(
    across(all_of(edge_types), mean),
    .by = c(num_vars, id_pairs, idx_rsmp, starts_with("thresh"))
  ) |> 
  pivot_longer(
    all_of(edge_types),
    names_to = "edge_type",
...
preds_sampled |>
  ...
  ggdist::stat_dots(side = "left") +
  geomtextpath::geom_texthline(
    aes(label = idx_name, yintercept = cor, color = idx_name),
    preds_main,
    hjust = 1
  ) +
...
scores_g |>
  filter(n() == 2, .by = c(num_vars, idx_rsmp)) |> 
  pivot_wider(
