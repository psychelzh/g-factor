...
targets::tar_load(c(cpm_pred, cpm_pred_main), store = store_g_invariance)
preds_main <- cpm_pred_main |>
  filter(gsr == "with") |>
  summarise(
...
dice_clean |>
  filter(binarize_method == "value", modal == "nbackfull") |>
...
# Predictable Brain Networks Analysis

```{r}
#| label: render-brain-mask
#| results: asis

targets::tar_load(brain_mask_main, store = store_g_invariance)
source(here::here("R/brain_mask.R"))
roi_labels <- prepare_roi_labels(
  readRDS(here::here("config/atlas_power.rds"))
)
config <- distinct(brain_mask_main, modal)
for (row in seq_len(nrow(config))) {
  knitr::knit_expand(
    "template/chord_network_tmpl.qmd",
    modal = config$modal[[row]]
  ) |>
