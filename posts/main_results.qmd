...
|---------------------|---------------------|------------------------------|
| UK Biobank (2004)         | 4                         | Cox S. R. et al., 2019                |
| HCP (2009)                | 12                        | Julien Dubois et al., 2018            |
| Aging Brain Cohort (2021) | 5 (part of NIH toolbox)   | Roger D. Newman-Norlund, et al., 2021 |
| ABCD Study (2015)         | 10 (7 from NIH toolbox)   | Wesley K. Thompson, et al., 2018      |
...
targets::tar_load(
  ...
  store = here::here("_targets/_experiment/bench_cpm")
)

cpm_pred_reg_covars2 |> 
  filter(filt == "bandpass", thresh_method == "alpha", thresh_level == 0.01) |> 
  pivot_longer(
...
targets::tar_load(cpm_pred_main_reg_covars2, store = store_cpm_main)
targets::tar_load(cpm_pred_reg_covars2, store = store_g_invariance)
preds_main <- cpm_pred_main_reg_covars2 |>
  filter(
    cond == "nbackrun1",
    parcel == "Power264",
    filt == "bandpass",
  ...
  summarise(
    across(all_of(names(edge_types)), mean),
...
preds_sampled <- cpm_pred_reg_covars2 |>
...
preds_sampled |>
  ...
  #   hjust = 1
  # ) +
  rlang::exec(scale_color_viridis_d, !!!scale_meas_behv) +
  facet_grid(
    cols = vars(edge_type),
    # cols = vars(cond),
    labeller = labeller(
...
dice_clean |>
  ...
  ggpubr::theme_pubclean() +
  ggh4x::facet_nested(
    edge_type ~ "Minimal Proportion" + binarize_level,
    labeller = labeller(
      edge_type = edge_types
    )
  ) +
...
dice_clean |>
  ...
  ggpubr::theme_pubclean() +
  ggh4x::facet_nested(
    edge_type ~ "Top N Edges" + binarize_level,
    labeller = labeller(
      edge_type = edge_types
    )
  ) +
...
targets::tar_load(cpm_pred_reg_covars2, store = store_task_selection)
...
targets::tar_load(
  c(cpm_pred_single_reg_covars2, data_names_ordered),
  store = store_task_selection
)
bind_rows(
  `Single Task` = cpm_pred_single_reg_covars2,
  `Intelligence` = cpm_pred_main_reg_covars2 |> 
      filter(
        parcel == "Power264",
        cond == "nbackrun1",
        filt == "bandpass",
...
for (row in seq_len(nrow(config))) {
  knitr::knit_expand(
    "template/chord_network_tmpl.qmd",
